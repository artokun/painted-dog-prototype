...
`
    ...
    contentTypes.items.forEach(contentType => {
      ...
      typeDefinitions += `}\n\n`
      typeDefinitions += `export type ${typeName} = Entry<${typeName}Fields>\n\n`
    })

    // Generate union type for all content types
    const allTypes = contentTypes.items.map(ct => toPascalCase(ct.sys.id)).join(' | ')
